...
def parse_args() -> argparse.Namespace:
    ...
    run_once.add_argument('--lookback-hours', type=int, default=72, help='Refetch this many hours on each symbol, default: 72')
    run_once.add_argument('--render-window-hours', type=int, default=24, help='Visible chart window in hours, default: 24')
    run_once.add_argument('--cum-window-hours', type=int, default=72, help='Rolling cumulative ΔCVD context window in hours, default: 72')
    ...
    rend.add_argument('--window-hours', type=int, default=24, help='Visible chart window in hours, default: 24')
    rend.add_argument('--cum-window-hours', type=int, default=72, help='Rolling cumulative ΔCVD context window in hours, default: 72')
...
def run_once_pipeline(base_dir: Path, universe_path: Path | None, interval: str, lookback_hours: int, symbols: list[str] | None, limit: int | None, output: Path, discord: bool, dry_run: bool = False, render_window_hours: int | None = None, cumulative_hours: int | None = None) -> RunOnceResult:
    ...
    compute_stats = compute_cvd_features(db_path, interval, market_keys=market_keys, limit=None)
    from .renderer import LAYOUT_VERSION, render_cvd_chart
    render_result = render_cvd_chart(
        db_path=db_path,
        universe_config_path=settings.markets_config_path,
        interval=interval,
        window_hours=render_window_hours or 24,
        output=output,
        cumulative_hours=cumulative_hours or max(lookback_hours, 72),
    ...
    summary = f"layout={LAYOUT_VERSION} selected={selected_markets_count} with_features={markets_with_feature_rows_count} plotted={plotted_series_count} omitted_crowding={omitted_for_crowding_count} skipped_no_features={skipped_no_feature_rows_count} skipped_no_cvd={skipped_no_usable_cvd_values_count} unresolved={unresolved_symbols_count} btc_price_market={render_result.btc_price_market} panel_plot_map={panel_plot_map_json} panel_omitted_map={panel_omitted_map_json} display_window_hours={render_window_hours or 24} cum_window_hours={cumulative_hours or max(lookback_hours, 72)} received={received_rows} computed={computed_rows} rendered={rendered_path}"
    discord_sent = False
...
def main() -> int:
    ...
    try:
        ...
        if args.command == 'compute':
            ...
            stats = compute_cvd_features(db_path, args.interval, market_keys=market_keys, limit=args.limit)
        ...
        if args.command == 'render':
            ...
            result = render_cvd_chart(
                ...
                cumulative_hours=args.cum_window_hours,
            ...
            print(f'layout={LAYOUT_VERSION} renderer={Path(__file__).resolve()} rendered={result.output_path} selected={result.selected_markets_count} with_features={result.markets_with_feature_rows_count} plotted={result.plotted_series_count} omitted_crowding={result.omitted_for_crowding_count} skipped_no_features={result.skipped_no_feature_rows_count} skipped_no_cvd={result.skipped_no_usable_cvd_values_count} unresolved={result.unresolved_symbols_count} btc_price_market={result.btc_price_market} panel_plot_map={panel_plot_map_json} panel_omitted_map={panel_omitted_map_json} display_window_hours={args.window_hours} cum_window_hours={args.cum_window_hours}')
            return 0
        ...
        if args.command == 'run-once':
            result = run_once_pipeline(
                ...
                render_window_hours=getattr(args, 'render_window_hours', None),
                cumulative_hours=getattr(args, 'cum_window_hours', None),
            )
